...
void state_only(
    ...
    TORCH_CHECK(
        g.dtype() == torch::kBFloat16,
        "g must be bfloat16"
    );

    TORCH_CHECK(
        beta.dtype() == torch::kBFloat16,
        "beta must be bfloat16"
    );

    TORCH_CHECK(
        A_log.is_cuda() &&
        A_log.is_contiguous(),
        "A_log must be contiguous CUDA"
    );

    TORCH_CHECK(
        A_log.dtype() == torch::kFloat32,
    ...
    TORCH_CHECK(
        ...
        "dt_bias must be contiguous CUDA"
    );
    ...
    TORCH_CHECK(
        final_state.is_cuda() &&
        final_state.is_contiguous(),
        "final_state must be contiguous CUDA"
    );

    TORCH_CHECK(
        final_state.dtype() == torch::kFloat32,
        "final_state must be float32"
    );
    ...
    if (has_mt) {
        ...
        TORCH_CHECK(
            mt.is_cuda() &&
            mt.is_contiguous(),
            "mt must be contiguous CUDA"
        );

        TORCH_CHECK(
            mt.dtype() == torch::kFloat32,
            "mt must be float32"
        );
    }
    ...
    TORCH_CHECK(
        cu_seqlens.dtype() == torch::kLong,
        "cu_seqlens must be int64"
    );

    TORCH_CHECK(
        num_warmup_chunks.is_cuda() &&
        num_warmup_chunks.is_contiguous(),
        "num_warmup_chunks must be contiguous CUDA"
    );

    TORCH_CHECK(
        num_warmup_chunks.dtype() == torch::kInt32,
        "num_warmup_chunks must be int32"
    );
    ...
    TORCH_CHECK(
        B == 1,
        "state_only requires B=1 (varlen mode)"
    );
    ...
    TORCH_CHECK(
        num_warmup_chunks.numel() == N,
        "num_warmup_chunks must have N elements"
    );
    ...
    TORCH_CHECK(
        ...
        final_state.size(2) == D &&
        final_state.size(3) == D,
        "final_state must be [N, H, D, D]"
    );
    ...
    int64_t total_tiles =
        (T_total + CHUNK - 1) / CHUNK + N;

    // Use the current winner's workspace layout.
    // It includes tile-prefix and ready flags in addition to the
    // original per-tile workspace.
    int64_t ws_bytes =
        get_workspace_size(
            T_total,
            H,
    ...
    auto warmup_ptr =
        num_warmup_chunks.data_ptr<int32_t>();

    cudaStream_t stream =
        at::cuda::getCurrentCUDAStream()
            .stream();

    // state_only does not consume q/Mqk.
    // Reuse k as a harmless dummy q.
    auto q_ptr = k_ptr;
    float scale = 1.0f;

    // Phase 1:
    // generate only workspace tiles belonging to each
    // CP segment's warmup suffix.
    ...
    // Phase 2:
    // compute h_t of every CP segment starting from h0=0.
    ...
    // Weak-decay fallback additionally needs transition matrices.
    if (has_mt) {
        auto mt_ptr =
            mt_out.value()
                .data_ptr<float>();
    ...
    m.def(
        ...
        py::arg("mt") = py::none(),
        py::arg("cu_seqlens"),
        py::arg("num_warmup_chunks")
    );

    m.def(
        "get_warmup_chunks",
        &get_warmup_chunks_cuda,
        "Get warmup chunks (CUDA)",
        py::arg("g"),
        py::arg("A_log"),
        py::arg("dt_bias"),
        py::arg("lower_bound"),
