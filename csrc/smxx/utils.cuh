...
//   - Warp-level iteration over all atoms in a rectangular state tile
...
__device__ void smem_cvt_fp32_to_bf16(
    ...
    constexpr int kBlockRows = Rows / kBlock;
    constexpr int kBlockCols = Cols / kBlock;
    constexpr int kTotalBlocks = kBlockRows * kBlockCols;
    ...
    for (int blk = warp_id; blk < kTotalBlocks; blk += num_warps) {
        int br = (blk / kBlockCols) * kBlock;
        int bc = (blk % kBlockCols) * kBlock;
        int e0 = lane_id * 2;
        int e1 = lane_id * 2 + 1;
